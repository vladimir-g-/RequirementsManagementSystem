...
async function renderPrefixSettings() {
  const result = await api('/api/requirement-prefixes');
  const projectPrefixes = result.prefixes.filter((item) => item.projectId === state.filters.projectId);
  app.innerHTML = `<div class="shell"><header class="topbar"><div class="topbar-start"><div class="brand">RMS<span>.</span></div>${projectPicker()}</div>${userMenu()}</header><main class="content"><div class="profile-heading"><div><p class="eyebrow">Администрирование</p><h1>Префиксы требований</h1><p class="page-note">Номер формируется автоматически по проекту и типу требования.</p></div><div class="heading-actions"><button class="clear" id="back-to-requirements">← К требованиям</button><button class="primary" id="save-prefixes">Сохранить изменения</button></div></div><div class="table-wrap"><table class="prefixes-table"><thead><tr><th>Тип требования</th><th>Префикс</th><th>Пример номера</th></tr></thead><tbody>${projectPrefixes.map((item) => `<tr><td>${esc(item.type)}</td><td><input class="prefix-input" data-project-id="${esc(item.projectId)}" data-type="${esc(item.type)}" value="${esc(item.prefix)}" maxlength="20"></td><td class="prefix-example">${esc(item.prefix)}-001</td></tr>`).join('')}</tbody></table></div><p class="error" id="prefix-error"></p></main></div>`;
  bindUserMenu(); bindPrefixProjectPicker();
  document.querySelector('#back-to-requirements').onclick = () => renderApp();
  document.querySelectorAll('.prefix-input').forEach((input) => { input.oninput = () => { input.closest('tr').querySelector('.prefix-example').textContent = `${input.value || 'REQ'}-001`; }; });
  document.querySelector('#save-prefixes').onclick = async () => {
    const editedPrefixes = new Map([...document.querySelectorAll('.prefix-input')].map((input) => [`${input.dataset.projectId}:${input.dataset.type}`, input.value.trim()]));
    const prefixes = result.prefixes.map((item) => ({ ...item, prefix: editedPrefixes.get(`${item.projectId}:${item.type}`) ?? item.prefix }));
    try { await api('/api/requirement-prefixes', { method: 'PUT', body: JSON.stringify({ prefixes }) }); await renderPrefixSettings(); } catch (error) { document.querySelector('#prefix-error').textContent = error.message; }
  };
}
function legacyOpenUserModal(item = null) {
